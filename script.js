...
if(!navigator.geolocation) {
    st.textContent = 'Geolocation is not supported by your browser';
} else {
   st.textContent = 'Locating…';
   navigator.geolocation.getCurrentPosition(success, error);
}
function success(position) {
    st.textContent = 'You Are at : Latitude = '+ position.coords.latitude + ', Longitude = '+position.coords.longitude;
}

function error() {
    st.textContent = 'Unable to retrieve your location';
}
